...
@Injectable()
export class CarBodyValidationPipe implements PipeTransform<any> {
  ...
  async transform(value: any, { metatype }: ArgumentMetadata) {
    ...
    const { name } = value;
    const carBody = await this.prismaService.carBodyType.findFirst({
      where: {
        name
      }
    })
    if (carBody) {
      throw new BadRequestException('Car Body with the provided name already exists.');
    }
