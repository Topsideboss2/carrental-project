...
@Injectable()
export class CarMakeValidationPipe implements PipeTransform<any> {
  ...
  async transform(value: any, { metatype }: ArgumentMetadata) {
    ...
    const { name } = value;
    const carMake = await this.prismaService.carMake.findFirst({
      where: {
        name
      }
    })
    if (carMake) {
      throw new BadRequestException('Car Make with the provided name already exists.');
    }
