...
@Injectable()
export class UserTypeValidationPipe implements PipeTransform<any> {
  ...
  async transform(value: any, { metatype }: ArgumentMetadata) {
    ...
    const { name } = value;
    const userType = await this.prismaService.userType.findFirst({
      where: {
        name
      }
    })
    if (userType) {
      throw new BadRequestException('User Type with the provided name already exists.');
    }
